...
- Les anomalies terrain iPadOS d'aperçu camera noir apres permission accordee,
  puis d'image visible brievement avant ecran noir, sont documentees. La
  correction 15.2.1 garantit un seul `getUserMedia` par session scanner reussie
  et reserve le fallback `{ video: true }` aux rejets avant obtention d'un flux ;
